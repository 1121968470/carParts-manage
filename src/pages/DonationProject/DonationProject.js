...
@connect(({ donationProject, loading }) => ({
  donationProject,
  loading: loading.models.donationProject,
}))
@Form.create()
class DonationProject extends PureComponent {
  state = {
    modalVisible: false,
  ...
  toEdit = record => {
    //跳转编辑页面
    router.push({
      pathname: '/donationProject/donationProject/donationProjectEdit',
      params: {
        data: record,
      },
    });
